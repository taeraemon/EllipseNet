...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='eldet',
    ...
    # ellipse loss
    self.parser.add_argument('--iou_weight', type=float, default=1,
                             help='loss weight for iou loss.')
    self.parser.add_argument('--ellipse_weight', type=float, default=1,
                             help='loss weight for ellipse regression.')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
    elif opt.task == 'eldet_bbox':
      # assert opt.dataset in ['coco_fhd']
      opt.heads = {'hm': opt.num_classes,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
  ...
  def init(self, args=''):
    default_dataset_info = {
      ...
      'ddd': {'default_resolution': [384, 1280], 'num_classes': 3, 
                ...
                'dataset': 'kitti'},
      'eldet': {'default_resolution': [512, 512], 'num_classes': 2, 
                'mean': [0.216, 0.216, 0.216], 'std': [0.222, 0.222, 0.222],
                'dataset': 'coco_fhd'},
    }
    class Struct:
