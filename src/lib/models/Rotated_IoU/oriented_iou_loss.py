...
def params2corners_th(box)-> torch.Tensor:
    """convert box coordinate to corners
...
def params2corners_sincos(box)-> torch.Tensor:
    """convert box coordinate to corners

    Args:
        box (torch.Tensor): (B, N, 6) with x, y, w, h, sin, cos
...
def cal_iou_sincos(box1:torch.Tensor, box2:torch.Tensor):
    """calculate iou

    Args:
        box1 (torch.Tensor): (B, N, 6) # sincos
        box2 (torch.Tensor): (B, N, 5) # alpha
    ...
    """
    corners1 = params2corners_sincos(box1)
...
def cal_diou_sincos(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    """calculate diou loss
    ...
    Args:
        ...
        box2 (torch.Tensor): [description]
    """
    iou, corners1, corners2, u = cal_iou_sincos(box1, box2)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    c2 = w*w + h*h      # (B, N)
    x_offset = box1[...,0] - box2[..., 0]
    y_offset = box1[...,1] - box2[..., 1]
