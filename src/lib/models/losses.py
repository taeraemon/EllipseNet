...
def angle_loss(pred_angle, true_angle, size_average=False, use_smooth_l1=False):
    ...
    mask[cos < 0] = -1
    # if cos >=0 else torch.atan2(-sin, -cos)
    sin, cos = sin * mask, cos * mask
    if use_smooth_l1:
        loss = F.smooth_l1_loss(torch.atan2(sin, cos),
                                torch.zeros_like(sin), size_average=False)
    else:
        loss = F.l1_loss(torch.atan2(sin, cos),
                         torch.zeros_like(sin), size_average=False)
    return loss
...
class RegL1Loss4Angle(nn.Module):
    ...
    def forward(self, output, mask, ind, target):
        ...
        mask = mask.unsqueeze(2).expand_as(pred).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
        loss = angle_loss(pred * mask, target * mask,
                          size_average=False, use_smooth_l1=self.use_smooth_l1)
        loss = loss / (mask.sum() + 1e-4)
        return loss
...
class AdaptiveWingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        loss1 = self.omega * \
            torch.log(1 + torch.pow(delta_y1 / self.epsilon, self.alpha - y1))
